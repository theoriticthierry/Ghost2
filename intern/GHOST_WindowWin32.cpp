	...
	m_nextWindow(NULL)
{
	...
	if (state != GHOST_kWindowStateFullScreen) {
		...
		m_hWnd = ::CreateWindow(
			...
			title.ReadPtr(),						// pointer to window name
	...
	if (m_wintab) {
		...
		if (fpWTInfo && fpWTInfo(0, 0, NULL)) {
			...
			if (fpWTOpen) {
				m_tablet = fpWTOpen( m_hWnd, (LPLOGCONTEXTA)  &lc, TRUE );
				if (m_tablet) {
...
{
	::SetWindowText(m_hWnd, title.ReadPtr());
}
...
GHOST_TSuccess GHOST_WindowWin32::setWindowPosition(GHOST_TUns32 x, GHOST_TUns32 y)
{
	...
	int cx = wBnds.getWidth();
	int cy = wBnds.getHeight();
	//return = ::SetWindowPos(m_hWnd, HWND_TOP, x, y, cx, cy, SWP_NOMOVE | SWP_NOZORDER) ? GHOST_kSuccess : GHOST_kFailure;
	if (::SetWindowPos(m_hWnd, HWND_TOP, x, y, cx, cy, SWP_ASYNCWINDOWPOS) )  //, SWP_NOMOVE | SWP_NOZORDER))
		return GHOST_kSuccess;
	else
		return GHOST_kFailure;
}
